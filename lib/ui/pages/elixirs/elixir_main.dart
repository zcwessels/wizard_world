...
class ElixirMain extends StatelessWidget {
  const ElixirMain({super.key});
          ...
          child: (context, state) {
            return Column(
              children: [
                ...
                if (state.main.elixirs?.isEmpty ?? true) Text("No Elixirs Found", style: theme.bodyMedium),
                if (state.main.filteredElixirs?.isEmpty ?? false) Text("No Matching Elixirs Found", style: theme.bodyMedium),
                Expanded(
                  child: ListView.separated(
                    shrinkWrap: true,
                    itemCount: state.main.filteredElixirs?.length ?? state.main.elixirs?.length ?? 0,
                    separatorBuilder: (BuildContext context, int index) => const SizedBox(height: 8),
                    itemBuilder: (BuildContext context, int index) {
                      WWElixir elixir = sl.wizardWorld.state.main.elixirs![index];
                      return InkWell(
                        ...
                        child: Container(
                          ...
                          child: Row(
                            children: [
                              ...
                              SizedBox(
                                width: 200,
                                child: Text(elixir.name, style: theme.bodyMedium, overflow: TextOverflow.ellipsis),
                              ),
                            ],
                          ),
                        ),
